...
function assign_page_info($title='')
{
	...
	$page_title.= $GLOBALS['blog_name'].' - 教育旅游 文化旅游 投资旅游 培训旅游 商业考察 友好城市建设旅游';
...
//插入评论者的名字
function insert_comments_name($arr)
{
	if ($_SESSION['user_id']>0)
	{
		$user=user_info();
		$content='<p>'.$arr["p1"].': '.$user['name'].'<br /><input type="hidden" name="name" id="name" value='.$user['name'].' /></p>'.
		'<input type="hidden" name="url" id="url" value="'.$user['home'].'" />'.
		'<input type="hidden" name="email" id="email" value="'.$user['email'].'" />';
	}
	else
	{
		$content='<p>'.$arr["p1"].': <br /><input type="text" name="name" id="name" /></p>';
		$content.='<p>'.$arr["p2"].': <br />'.'<input type="text" name="url" id="url"  /></p>';
