...
{
	$current_template_info=get_template_info($template_name);
...
elseif ($action=='select_template')
{
	...
	//将配置读出，并用正则替换模板设置的值
	$blog_config=file_get_contents(PBBLOG_ROOT.'home/data/config.php');
	$pattern="/template_name   = (.+)/";
	$replace="template_name   = '".$template_name."';";
	$blog_config=preg_replace($pattern,$replace,$blog_config);
	...
	fclose($fp);
	show_message('模板设置成功','admin.php?act=templates_list');
