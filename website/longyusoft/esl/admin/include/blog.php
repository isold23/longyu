...
elseif ($action=='del_blog')
{
	...
	$cat_id=intval($db->getone('SELECT cat_id FROM '.table('blog')." WHERE id='".$blog_id."'"));
	$db->query('UPDATE ' . table('blog_category') . " SET blog_count = blog_count - 1 WHERE id = '$cat_id'");
...
elseif ($action=='act_add_blog')
{
	...
	$lang_select=$_POST['lang_select'];				//1表示中文
	$str_lang = "cn";
	if($lang_select == 1)
	{
		$str_lang = "en";
	...
	" (`id` ,`user_id` ,`cat_id` ,`title` ,`description` ,`content` ,`post_time` ,`last_time` ,`comment_count` ,`view_count` ,`password`,`view_group`,`open_type`,`url_type`,`lang` )
		VALUES (NULL , '".$user_id."', '".$cat_id."', '".$blog_title."', '".
	$desc."', '".$content."', '".$time."', '0', '0', '0', '".$password."', '".
	$group."','".$open_type."','".$url_type."','".$str_lang."')";
	if ($db->query($sql))
	{
		//开始ping
...
elseif ($action=='act_edit_blog')
{
	...
	"' , `last_time`='".$time."', `cat_id`='".$cat_id."' , `password`='".$password.
	"' , `open_type`='".$open_type."' , `view_group`='".$group."' , `url_type`='".$url_type."' , `lang`='".$str_lang.
	"' WHERE id='".$blog_id."'";
