...
if ($action=='friend_link_list')
{
	...
	$page_size=!empty($pager_size)?$page_size:'15';
	$sql='SELECT count(*) FROM '.table('blog_link').' WHERE 1';
	$page_count=intval(($db->getone($sql)-1)/$page_size)+1;
	$page_arr=create_page($page_count,$pg,0);

	$start=($pg-1)*$page_size;
	$sql='SELECT * FROM '.table('blog_link').' WHERE 1 '.
	" ORDER BY lnk_sort ASC LIMIT ".$start.' , '.$page_size;
...
elseif ($action=='edit_friend_link')
{
	...
	$sql='SELECT * FROM '.table('blog_link')." WHERE link_id= '".$id."'";
...
elseif ($action=='act_edit_friend_link')
{
	...
	if ($db->query($sql))
	{
		...
		show_message('编辑友情链接成功','admin.php?act=friend_link_list');
	}
	else
	{
...
elseif ($action=='act_add_friend_link')
{
	...
	$sql='INSERT INTO '.table('blog_link').
	" (`link_id` ,`title` ,`desc` ,`url` ,`sort`  )
		VALUES (NULL , '".$module_title."', '".$desc."', '".$module_content."', '".$sort."')";
	if ($db->query($sql))
	{

		show_message('添加友情链接成功','admin.php?act=friend_link_list');
	}
	else
	{
...
{
	$module_id=$_GET['id'];
	$sql='DELETE FROM '.table('blog_link')." WHERE link_id='".$module_id."'";
	if ($db->query($sql))
	{
