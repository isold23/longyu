...
elseif ($action=='act_edit_nav')
{
	...
	{
		show_message('请填写NAV标题',$referer_url);
	}

	if (empty($module_content))
	{
		show_message('请填写URL',$referer_url);
	}
	...
	if ($db->query($sql))
	{
		...
		show_message('编辑NAV成功','admin.php?act=nav_list');
	}
	else
	{
		show_message('编辑NAV失败，请重新返回添加',$referer_url);
	}
}
...
elseif ($action=='act_add_nav')
{
	...
	if ($db->query($sql))
	{
		...
		show_message('添加网站导航链接成功','admin.php?act=nav_list');
	}
	else
	{
