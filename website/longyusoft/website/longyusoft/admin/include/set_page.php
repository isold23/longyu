...
if ($action=='set_footer')
{
	...
	$footer=file_get_contents(PBBLOG_ROOT.'/themes/'.$template_name.'/'.$get_file);
	...
	/* 获得模板文件列表 */
	$templates = array();
	$template_dir        = @opendir(PBBLOG_ROOT.'/themes/'.$template_name.'/');
	while ($file = readdir($template_dir))
	{
		if ($file != '.' && $file != '..' && file_exists(PBBLOG_ROOT.'/themes/'.$template_name.'/'. $file))
		{
			$point = strrpos($file, '.');
...
elseif ($action=='get_page_data')
{
	...
	$data=file_get_contents(PBBLOG_ROOT.'/themes/'.$template_name.'/'.$file);
...
elseif ($action=='ajax_post_page_data')
{
	...
	$fp=@fopen(PBBLOG_ROOT.'/themes/'.$template_name.'/'.$file,"w") or $res['error']='无法写入文件，请检查文件是否有权限';
	flock($fp,LOCK_EX);
	fwrite($fp,$data);
	fclose($fp);
...
elseif ($action=='act_set_page')
{
	...
	$fp=@fopen(PBBLOG_ROOT.'/themes/'.$template_name.'/'.$file,"w") or die('can not open file');
	flock($fp,LOCK_EX);
	fwrite($fp,$data);
	fclose($fp);
	
	show_message('页面修改成功','admin.php?act=set_footer&file=&'.$file);
