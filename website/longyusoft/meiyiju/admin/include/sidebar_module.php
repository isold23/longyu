...
elseif ($action=='act_edit_sidebar')
{
	...
	{
		show_message('请填写边栏标题',$referer_url);
	}

	if (empty($module_content))
	{
		show_message('请填写边栏内容',$referer_url);
	}
	...
	{
		make_sidebar();
		show_message('编辑插件成功','admin.php?act=sidebar_list');
	}
	else
	{
		show_message('编辑插件失败，请重新返回添加',$referer_url);
	}
}
...
elseif ($action=='sidebar_setup_list')
{
	...
	/* 获得可安装的边栏模块 */
	$modules = array();
	$sidebar_dir        = @opendir(PBBLOG_ROOT . '/include/modules/sidebar/');
	while ($file = readdir($sidebar_dir))
	{
		if ($file != '.' && $file != '..' && file_exists(PBBLOG_ROOT. '/include/modules/sidebar/' . $file))
		{
			include_once(PBBLOG_ROOT. '/include/modules/sidebar/' . $file);
...
elseif ($action=='setup_sidebar')
{
	...
	if ($type!=1||is_sidebar($id))
	{
		show_message('该插件已经安装',$referer_url);
	}
...
elseif ($action=='act_setup_sidebar')
{
	...
	{
		make_sidebar();
		show_message('安装插件成功','admin.php?act=sidebar_setup_list');
	}
	else
	{
		show_message('安装插件失败，请重新返回添加',$referer_url);
	}
}
...
function make_sidebar()
{
	...
	$fp=fopen(PBBLOG_ROOT.'/themes/'.$GLOBALS['template_name'].'/sidebar.html',"w") or die('can not open file');
	flock($fp,LOCK_EX);
