<?php 
  ...
  /*$cache_index_pic=md5("index_pic");
	...
	$sql = "select a.id,a.title,a.post_time,b.name from ".table('blog')." AS a INNER JOIN ".table('blog_category')." AS b ON a.cat_id = b.id ORDER BY post_time DESC LIMIT 8";
	...
	$sql = "select a.id,a.name as aname, a.upload_time, b.name as bname from ".table('resource')." AS a INNER JOIN ".table('resource_category')." AS b ON a.cat_id = b.id ORDER BY upload_time DESC LIMIT 7";
	$last_resource_list=$db->getall($sql);
	$smarty->assign('last_resource_list',$last_resource_list);
	
	//获取最新资源
	$cache_id = md5("hot_resource_list");
	$sql = "select a.id,a.name as aname,b.name as bname from ".table('resource')." AS a INNER JOIN ".table('resource_category')." AS b ON a.cat_id = b.id ORDER BY view_time DESC LIMIT 7";
