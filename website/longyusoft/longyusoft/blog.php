...
if($cat_id > 0){
	$sql = "select a.id,a.cat_id,a.title,a.post_time,a.tag,a.author,a.description,a.content,a.comment_count,a.trackback_count,a.view_count,b.name from ".table('blog')." AS a INNER JOIN ".table('blog_category')." AS b ON a.cat_id = b.id WHERE a.cat_id=".$cat_id." ORDER BY a.post_time DESC LIMIT ".$start.' , '.$page_size;
}else{
	$sql = "select a.id,a.cat_id,a.title,a.post_time,a.tag,a.author,a.description,a.content,a.comment_count,a.trackback_count,a.view_count,b.name from ".table('blog')." AS a INNER JOIN ".table('blog_category')." AS b ON a.cat_id = b.id ORDER BY a.post_time DESC LIMIT ".$start.' , '.$page_size;
}
