...
$cat_id=isset($_GET['cat'])?intval($_GET['cat']):0;
	...
	if($cat_id > 0)
	{
		$sql = "select a.id,a.cat_id,a.name AS aname,a.upload_time,a.author,a.path,a.image_path,a.view_time,b.name AS bname from ".table('resource')." AS a INNER JOIN ".table('resource_category')." AS b ON a.cat_id = b.id WHERE a.cat_id=".$cat_id." ORDER BY a.upload_time  DESC LIMIT ".$start.' , '.$page_size;
	}
	else
	{
		$sql = "select a.id,a.cat_id,a.name AS aname,a.upload_time,a.author,a.path,a.image_path,a.view_time,b.name AS bname from ".table('resource')." AS a INNER JOIN ".table('resource_category')." AS b ON a.cat_id = b.id ORDER BY a.upload_time  DESC LIMIT ".$start.' , '.$page_size;
	}
	$resource_list=$db->getall($sql);
	
		foreach ($resource_list as $key=>$val){
		$resource_list[$key]['aname'] = utfSubstr($resource_list[$key]['aname'], 0,30,2);
	}
...
else
{
	...
	$sql = "select a.id,a.cat_id,a.name AS aname,a.upload_time,a.author,a.path,a.image_path,a.view_time,a.remark,b.id AS bid,b.name AS bname from ".table('resource')." AS a INNER JOIN ".table('resource_category')." AS b ON a.cat_id = b.id WHERE a.id=".$id;
