...
elseif ($action=='act_add_resource')
{
	...
	$fine_radio = $_POST['fine_radio'];
	$is_fine=0;
	if($fine_radio == 'yes_radio')
	{
		$is_fine = 1;
	}
	...
	" (`id` ,`user_id` ,`cat_id` ,`name` ,`is_fine`,`number` ,`spec` ,`upload_time` ,`origin` ,`material` ,`weight` ,`price` ,`image_path`,`remark` )
		VALUES (NULL , '".$user_id."', '".$cat_id."', '".$resource_title."', '".$is_fine."', '".
	$resource_number."', '".$resource_spec."', '".$time."', '".$resource_origin."', '".$resource_material."','".$resource_weight."', '".$resource_material."', '".$resource_image_path."', '".$resource_remark."')";
	if ($db->query($sql))
	{
		//开始ping
...
elseif ($action=='act_edit_resource')
{
	...
	"' ,`remark`='".$resource_remark."',`is_fine`='".$is_fine."',`image_path`='".$resource_image_path."', `upload_time`='".$time."', `cat_id`='".$cat_id."' ,`origin`='".$resource_origin."' , `material`='".$resource_material."' , `weight`='".$resource_weight."' , `price`='".$resource_price.
