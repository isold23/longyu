...
namespace sqlite3x {
	...
	private:
		...
		struct sqlite3 *db;
	private:
		sqlite3_connection(const sqlite3_connection &src);
		sqlite3_connection& operator=(const sqlite3_connection &src);
	...
	private:
		...
		bool intrans;
	private:
		sqlite3_transaction( const sqlite3_transaction &src );
		sqlite3_transaction& operator=(const sqlite3_transaction &src );
	...
	private:
		...
		int argc;
	private:
		sqlite3_command( const sqlite3_command &src );
		sqlite3_command& operator=( const sqlite3_command &src );
