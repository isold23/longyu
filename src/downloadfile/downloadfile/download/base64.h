...
public:
        CBase64(void){}
        ~CBase64(void){}
        static int Encode(char *DataOut, char *DataIn, int size);
        static int Decode(char *DataOut, char *DataIn, int size);
private:

};

inline int CBase64::Encode (char *DataOut, char *DataIn, int size)
{
  ...
  int nTemp = static_cast<int>(pBuffer - DataOut);
  return nTemp;
}

inline int CBase64::Decode (char* DataOut, char* DataIn, int size)
{
  int nDstLen;                  // .......
  int nValue;                   // ........
  int i;
  ...
  while (i < size)
    {
      if (*DataIn != '\r' && *DataIn != '\n')
        {
          nValue = DeBase64Tab[*DataIn++] << 18;
          nValue += DeBase64Tab[*DataIn++] << 12;
          *DataOut++ = (nValue & 0x00ff0000) >> 16;
          nDstLen++;

          if (*DataIn != '=')
            {
              nValue += DeBase64Tab[*DataIn++] << 6;
              *DataOut++ = (nValue & 0x0000ff00) >> 8;
              nDstLen++;

              if (*DataIn != '=')
                {
                  nValue += DeBase64Tab[*DataIn++];
                  *DataOut++ = nValue & 0x000000ff;
                  nDstLen++;
                }
            }
