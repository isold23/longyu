...
void ReportStats() {
	...
	for (int i=0; i<S_NUM_STATS; ++i) {
		len += sprintfn(buffer, ARRAY_SIZE(buffer), "%s%s:%d",
...
PseudoTcp::NotifyClock(uint32 now) {
	...
	if ((m_snd_wnd == 0) 
		&& (TimeDiff(m_lastsend + m_rx_rto, now) <= 0)) 
	{
...
PseudoTcp::Recv(char * buffer, size_t len) {
	...
	if ((sizeof(m_rbuf) - m_rlen - m_rcv_wnd) 
		>= _min<uint32>(sizeof(m_rbuf) / 2, m_mss)) 
	{
...
PseudoTcp::clock_check(uint32 now, long& nTimeout) 
{
	...
	if (m_t_ack) {
		nTimeout = _min(nTimeout, 
			TimeDiff(m_t_ack + ACK_DELAY, now));
	}
	if (m_rto_base) {
		nTimeout = _min(nTimeout, 
			TimeDiff(m_rto_base + m_rx_rto, now));
	}
	if (m_snd_wnd == 0) {
