...
BOOL CADODatabase::Open(LPCTSTR lpstrConnection, LPCTSTR lpstrUserID, LPCTSTR lpstrPassword)
{
	...
	if(strcmp((const char*)lpstrConnection, ("")) != 0)
	...
	try
	{
		...
		m_pConnection->Provider = (_bstr_t(provider));
		hr = m_pConnection->Open(_bstr_t(m_strConnection), _bstr_t(lpstrUserID), _bstr_t(lpstrPassword), adModeUnknown);
		return hr == S_OK;
	}
	catch(_com_error &e)
	{
...
{	
	...
	if(strcmp((const char*)lpstrExec, ("")) != 0)
	...
	try
	{
		...
		if(bIsSelect || nOption == openQuery || nOption == openUnknown)
			m_pRecordset->Open((LPCSTR)m_strQuery.GetBuffer(0), _variant_t((IDispatch*)mpdb, TRUE), 
		...
		{
			PrintLog(_LOG_LEVEL_DEBUG,  "Unknown parameter. %d", nOption);
			return FALSE;
		}
	}
	catch(_com_error &e)
...
// user unicode
BOOL CADORecordset::Open(_ConnectionPtr mpdb, wchar_t *pwszExec, int nOption)
{
	Close();
	
	bstr_t bstrtem = pwszExec;
	...
	{
		m_pCmd->ActiveConnection = mpdb;
		m_pCmd->CommandText = bstrtem;

		m_pRecordset->CursorType = adOpenStatic;
		m_pRecordset->CursorLocation = adUseClient;
		if(bIsSelect || nOption == openQuery || nOption == openUnknown)
		{
			m_pRecordset->Open ((IDispatch *) m_pCmd, vtMissing, adOpenForwardOnly,
...
BOOL CADORecordset::OpenSchema(int nSchema, LPCTSTR SchemaID)
{
	try
	{
		...
		if(strlen((const char*)SchemaID) != 0)
			vtSchemaID = SchemaID;
...
#include "atlconv.h"
BOOL CADORecordset::GetFieldValue(LPCTSTR lpFieldName, wchar_t *pwszValue, CString strDateFormat)
{
	CString str = _T("");
	_variant_t vtFld;
	...
	try
	{
		...
		case VT_R8:
			...
			break;
		case VT_BSTR:
			wcscpy(pwszValue, vtFld.bstrVal);
			return TRUE;
			break;
		case VT_I2:
		case VT_UI1:
		...
		USES_CONVERSION; 
		pwszValue = A2W((LPCSTR)(str.GetBuffer(0)));
		return TRUE;
	}
	catch(_com_error &e)
	{
...
{
	ASSERT(m_pConnection != NULL);
	ASSERT(strcmp((const char*)lpstrExec, ("")) != 0);
	_variant_t vRecords;
...
BOOL CADORecordset::GetFieldInfo(FieldPtr pField, CADOFieldInfo* fldInfo)
{
	...
	strcpy(fldInfo->m_strName, (const char*)pField->GetName());
...
BOOL CADORecordset::GetChunk(LPCTSTR lpFieldName, LPCTSTR lpFilePath)
{
	FieldPtr pField = m_pRecordset->Fields->GetItem(lpFieldName);
	
	try
	{
		long nSize = pField->ActualSize;

		if(nSize > 0)
		{
			_variant_t	varBLOB;
			varBLOB = pField->GetChunk(nSize);
			if(varBLOB.vt == (VT_ARRAY | VT_UI1))
			{
				...
				{	
					...
					if(!l_file.Open(lpFilePath, CFile::typeBinary | CFile::modeCreate | CFile::modeWrite, &l_e))
					{
						delete[] pBuffer;
						return FALSE;
					}
					
					l_file.Write(pBuffer, nSize);
...
CString CADORecordset::GetString(LPCTSTR lpCols, LPCTSTR lpRows, LPCTSTR lpNull, long numRows)
{
	...
	if(strlen((const char*)lpCols) != 0)
		varCols = _bstr_t(lpCols);

	if(strlen((const char*)lpRows) != 0)
...
BOOL CADORecordset::Find(LPCTSTR lpFind, int nSearchDirection)
{
	...
	{
		PrintLog(_LOG_LEVEL_DEBUG, "Unknown parameter. %d", nSearchDirection);
...
long CADORecordset::GetFieldContentSize(LPCTSTR lpField)
{
	FieldPtr pField = m_pRecordset->Fields->GetItem(lpField);
...
BOOL CADORecordset::AppendChunk(LPCSTR strFieldName, LPCTSTR lpszFileName)
{
	...
	if (file.Open(lpszFileName, CFile::modeRead))
	{
		...
		// CString slong;
		// slong.Format("文件长度%ld",lpszFileName);
	 
		if (pbuf != NULL && file.Read(pbuf, length) == (DWORD)length)
		{ 
			//bret = AppendChunk(strFieldName, pbuf, length);///////////////////////////有错误暂时注释掉/////////////////////////////////////////////
		}
		if (pbuf != NULL) 
			delete[] pbuf;
	}
