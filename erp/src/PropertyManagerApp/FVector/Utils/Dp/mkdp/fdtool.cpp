...
uint8_t fdTypeIndexOf(const char *s)
{
...
void dparse(const char *str,std::vector<DFStructDesc> &sts)
{ 
	DFStructDesc st;
	DFMemberDesc member;
	...
	while (st_key_start!=NULL && *st_key_start!='\0')
	{
		...
		while (member_id_start!=NULL && member_id_start<st_key_end)
		{
			...
			{
				char *member_default_start = strchr(member_default_key_start+cstr_len(kw_default),'(');
