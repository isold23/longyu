...
bool CThreadGroup::StopAll()
{
	...
	if(liTermainte)
	{
		//TRACE(1,"CThreadGroup::StopAll 强行结束线程数量" << liTermainte<< " 正常结束数量:" << liThread-liTermainte);
	}
	else
	{
...
void CThreadGroup::PrintThreadInfo(STRU_THREAD_CONTEXT& aoThreadContext)
{
	...
	//打印线程信息
	//TRACE(1,"CThreadGroup::Dump 线程ID:" << aoThreadContext.muiThreadID<< "句柄:" << aoThreadContext.mhHandle<< "线程名称:" << aoThreadContext.moThreadStat.GetThreadName()<< "性能:" << aoThreadContext.moThreadStat.GetPerformance()<< "优先级:" << aoThreadContext.miPriority);
	...
	if(GetThreadTimes(aoThreadContext.mhHandle, &lstruCreationTime, &lstruExitTime, &lstruKernelTime, &lstruUserTime))
	{
		...
		if(GetExitCodeThread(aoThreadContext.mhHandle, &ldwExitCode))
		{
			...
			{
				//TRACE(1, "CThreadGroup::Dump 线程没有退出，存活计数:"<< aoThreadContext.moThreadStat.GetAliveCounter());
			}
		}	
...
void CThreadGroup::PrintThreadTime(char* apTime, FILETIME& aoFileTime)
{
	...
	//TRACE(1,"CThreadGroup::Dump " << apTime << " " << lstruSystemTime.wYear << "-" << lstruSystemTime.wMonth << "-" << lstruSystemTime.wDay << "-" << lstruSystemTime.wHour << "-" << lstruSystemTime.wMinute << "-" << lstruSystemTime.wSecond << "-" << lstruSystemTime.wMilliseconds);
}
#endif
...
void CThreadGroup::PrintThreadInfo(STRU_THREAD_CONTEXT& aoThreadContext)
{
	...
	//打印线程信息
	//TRACE(1,"CThreadGroup::Dump " << "线程ID:" << aoThreadContext.muiThreadID<< " 线程名称:" << aoThreadContext.moThreadStat.GetThreadName()<< " 性能:" << aoThreadContext.moThreadStat.GetPerformance()<< " 优先级:" << aoThreadContext.miPriority);
	...
	{
		//TRACE(1,"CThreadGroup::Dump 线程已经退出，存活计数:"<< aoThreadContext.moThreadStat.GetAliveCounter());
	}
}
#endif
