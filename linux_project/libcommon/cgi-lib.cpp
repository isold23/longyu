...
$Id: cgi-lib.c,v 1.1.1.1 2004/02/13 07:21:11 hunter Exp $
...
All Rights Reserved
*/
#ifndef __CGI_LIB_C__
#define __CGI_LIB_C__
...
char* get_POST()
{
    ...
    {
        unsigned int content_length = atoi(CONTENT_LENGTH);
...
char* get_GET()
{
    ...
    return (char*)newstr(QUERY_STRING);
}
...
int parse_form_encoded(llist *entries)
{
    ...
    char *uploadfname = NULL, *tempstr = NULL, *boundary = NULL;
    ...
    while ((bytesread=cgigetline(buffer,BUFSIZ)) != 0)
    {
        start = 1;
            ...
            if (bytesread && strstr(buffer,boundary) == NULL)
            {
                ...
                else
                {
                    ...
                    while (i < prevbytesread)
                    {
                        if (isfile)
                        {
                            if ((iTotalWrite < iUploadFileMaxSize) || (iUploadFileMaxSize <= 0))
                            {
...
int read_cgi_input(llist *entries)
{
    ...
    int status = parse_CGI_encoded(entries, input);
    free(input);
...
char **cgi_val_multi(llist l, char *name)
{
    ...
    if (FOUND)
    {
        ...
        char **ret_val = (char**) malloc(sizeof(char*) * (num_vals + 1));
        window = l.head;
        int i = 0;
        while (window != NULL)
        {
