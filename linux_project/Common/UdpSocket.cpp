...
参数说明：
	...
	dst_port:数据包目标端口。
************************************************************************/
bool CUdpSocket::SendData(const char* buffer, const unsigned int length, 
	uint32 dst_ip, const unsigned short dst_port){
	sockaddr_in sock_addr;
	memset(&sock_addr, 0, sizeof(sockaddr_in));
	CCommon::SetSocketAddr(sock_addr, htonl(dst_ip), htons(dst_port));
	uint32 nSend  = 0;
	char send_buffer[DEF_UDP_PACK_BUF_LEN] = {0};
	int send_len = DEF_UDP_PACK_BUF_LEN;
...
************************************************************************/
bool CUdpSocket::RecvData(char* buffer, unsigned int &length,
	uint32 &src_ip, unsigned short &src_port){
	//uint32 auiRecvLen = 0;
	sockaddr_in sock_addr;
	memset(&sock_addr, 0, sizeof(sockaddr_in));
	//uint32 sock_addr_size = 0;
	uint32 sockaddr_len = sizeof(sockaddr_in);

	char recv_buffer[DEF_UDP_PACK_BUF_LEN] = {0};
	int32 recv_len = DEF_UDP_PACK_BUF_LEN;
	...
	if(recv_len <= 0){
		...
		int buffer_len = 0;
		CNetPackVersion1 pack;
		pack.Unpack((const char*)recv_buffer, (const int)recv_len, buffer, buffer_len, data_len);
		length = buffer_len;
...
参数说明：
	...
	dst_port:数据包目标端口。
************************************************************************/
bool CUdpSocket::SendDataWithoutHeader(const char* buffer, const unsigned int length, 
	const uint32 dst_ip, const unsigned short dst_port){
	sockaddr_in sock_addr;
	memset(&sock_addr, 0, sizeof(sockaddr_in));
	CCommon::SetSocketAddr(sock_addr, htonl(dst_ip), htons(dst_port));
	int32 nSend  = sendto(miSocket,buffer, length, 0,(sockaddr *) &sock_addr, sizeof(sockaddr_in));
	return true;
}
...
参数说明：
	...
	src_port:数据包来源端口，本地字节序。
************************************************************************/
bool CUdpSocket::RecvDataWithoutHeader(char* buffer, unsigned int &length, 
	uint32 &src_ip, unsigned short &src_port){
	//uint32 auiRecvLen = 0;
	sockaddr_in sock_addr;
	memset(&sock_addr, 0, sizeof(sockaddr_in));
