...
bool CTcpStream::Connect(const char* apszDestIp,unsigned short ausPort,int liTimeout)
{
	if(miSocket <= 0)
	{
		return false;
...
bool CTcpStream::SendData(const char* apData,unsigned short ausDataLen)
{
	return (ausDataLen == send(miSocket,apData,ausDataLen,0));
}
int CTcpStream::RecvData(char* apBuff,unsigned short ausBuffSize,int liTimeout)
{
