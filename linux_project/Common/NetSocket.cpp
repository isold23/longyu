...
int CNetSocket::Accept(unsigned int &ip, unsigned short &port){
	int err = 0;
...
int CNetSocket::SendData(const char* buffer, const int length){
	...
	int liSendLen = m_pNetPack->_max_pack_size;
	int nRet = m_pNetPack->Pack(buffer, length, lszSendBuffer, liSendLen);
	if(nRet<0){
		TRACE(1, "CNetSocket::SendData 打包失败。fd = "<<miSocket<<" ret = "<<nRet);
		return -1;
	}
...
int CNetSocket::SendCacheData(){
	if(miResendLength != 0){
		...
		if(nRet < 0){
			TRACE(1, "CNetSocket::SendCacheData 发送数据异常1。nRet = "<<nRet<<" miResendLength = "<<miResendLength);
			return -1;
		}
		...
		if(nRet > miResendLength){
			mbCanSend = false;
			TRACE(1, "CNetSocket::SendCacheData 发送数据异常2。nRet = "<<nRet<<" miResendLength = "<<miResendLength);
			return -1;
		}
	}
	miResendLength = 0;
	...
	while(moNetStat == COMMON_TCP_ESTABLISHED || moNetStat == COMMON_TCP_CONNECTED){
		...
		if(nRet > 0){
			...
			while (miRecvCacheLength > m_pNetPack->_min_pack_size){
				...
				int liRecvBufferLen = m_pNetPack->_max_pack_size;
				int liRecvDataLen = 0;
