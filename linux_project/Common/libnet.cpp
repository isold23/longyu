...
string GetAddrStr(const sockaddr_in& aoAddr)
{
	char lszBuff[32] = {0};
...
bool Bind(int aiSocket,const char* apszAddr,unsigned short ausPort)
{
...
bool ConnectToSvr(int aiSocket,const char* apszAddr,unsigned short ausPort)
{
	//set svr addr
	struct sockaddr_in loAddr;
	memset(&loAddr, 0,sizeof(loAddr));
...
bool CreateTcpServer(int aiSocket,const char* apszAddr,unsigned short ausPort,
...
void SetMaxOpenFiles(unsigned int auiMaxFileNums)
{
#ifndef WIN32
...
//根据host获取ip
bool GetIpByHostName(const string& astrHost, string& astrIp)
{
